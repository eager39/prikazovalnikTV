                                 ...
                                 if (type == "html") {
                                    ...
                                    fs.remove(__dirname + '/../src/assets/' + name, function(err) {
                                       if (err) {
                                          ...
                                          var deletethis
                                          name = name.split(".htm", 1)

                                          var test = new RegExp("^" + display)
                                          folder = name[0].replace(test, "")
                                          //console.log("!!!!!",name)
                                          //var folder=name[1].replace(/^([0-9]+)/, "")
                                          console.log(folder + "folder")
                                          fs.readdir(__dirname + '/../src/assets/', function(err, items) {
                                             console.log(items);

                                             for (var i = 0; i < items.length; i++) {

                                                if (items[i].includes(folder && ("_datoteke" || "_files"))) {
                                                   deletethis = items[i]
                                                }
                                             }
                                             fs.remove(__dirname + '/../src/assets/' + deletethis, function(err) {
